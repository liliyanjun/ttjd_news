...
app.controller('loginCtrl', function ($scope, httpReq, auth, $location, localStorage, $interval, showToast) {
    ...
    $scope.login_info = {
        username: localStorage.get("ttjd_manage_username")
    };
    ...
    $scope.login = function () {
        var username = $scope.login_info.username;
        var password = $scope.login_info.password;
        if (!username) {
            showToast("请输入手机号");
            return;
        }
        ...
        auth.login(username, password).then(function () {
            localStorage.set("ttjd_manage_username", username);
